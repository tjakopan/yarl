...
class RetryPolicyBuilder<R> : RetryPolicyBuilderBase<R, RetryPolicyBuilder<R>>() {
  ...
  fun retry(retryCount: Int, onRetry: (Result<R>, Int, Context) -> Unit): RetryPolicy<R> {
  ...
  fun retry(retryCount: Int): RetryPolicy<R> {
    val doNothing: (Result<R>, Int, Context) -> Unit = { _, _, _ -> Unit }
    return retry(retryCount, doNothing)
  }

  fun retry(onRetry: (Result<R>, Int, Context) -> Unit): RetryPolicy<R> = retry(1, onRetry)

  fun retry(): RetryPolicy<R> = retry(1)

  fun retryForever(onRetry: (Result<R>, Int, Context) -> Unit): RetryPolicy<R> {
    this.onRetry = { outcome, _, i, ctx -> onRetry(outcome, i, ctx) }
    return RetryPolicy(this)
  }
  ...
  fun waitAndRetry(retryCount: Int, sleepDurationProvider: (Int, Result<R>, Context) -> Duration): RetryPolicy<R> {
  ...
  fun waitAndRetryForever(sleepDurationProvider: (Int, Result<R>, Context) -> Duration): RetryPolicy<R> {
    val doNothing: (Result<R>, Duration, Int, Context) -> Unit = { _, _, _, _ -> Unit }
